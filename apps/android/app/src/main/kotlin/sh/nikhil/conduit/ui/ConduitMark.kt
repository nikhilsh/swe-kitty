...
fun ConduitMark(size: Dp, modifier: Modifier = Modifier, color: Color? = null) {
    Canvas(modifier = modifier.size(size)) {
        ...
        val outline = if (color != null) {
            SolidColor(color)
        } else {
            Brush.linearGradient(listOf(MarkCyan, MarkGreen), start = p(4f, 4f), end = p(28f, 28f))
        }
        ...
        drawRoundRect(color = color ?: MarkCyan, topLeft = p(14.4f, 4.4f), size = Size(3.2f * s, 2f * s), cornerRadius = CornerRadius(1f * s))
        drawRoundRect(color = color ?: MarkGreen, topLeft = p(14.4f, 25.6f), size = Size(3.2f * s, 2f * s), cornerRadius = CornerRadius(1f * s))
        ...
        drawPath(face, color = MarkEye, style = Stroke(width = 1.7f * s, cap = StrokeCap.Round, join = StrokeJoin.Round))
    }
}
