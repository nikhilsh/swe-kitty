...
/// Pins down the `ConduitChatView` events-derivation contract — i.e.
/// what `ConduitUI.ChatViewModel.mergedEvents(conversation:chatLog:)`
...
@Suite("ConduitUI.ChatViewModel.mergedEvents")
struct ConduitChatIngestTests {
    ...
    @Test func emptyChatLogReturnsConversationUnchanged() {
        ...
        let merged = ConduitUI.ChatViewModel.mergedEvents(
            conversation: convo,
            chatLog: []
        )
    ...
    @Test func toolRoleSyntheticItemsCarryToolKind() {
        ...
        // ConversationItem so ConduitEventRow routes it to ConduitToolCard
        // instead of ConduitChatMessageRow (matches the legacy fallback
