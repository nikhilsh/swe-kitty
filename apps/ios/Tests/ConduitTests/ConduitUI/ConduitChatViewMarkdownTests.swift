...
/// Smoke tests for the markdown pipeline now driving ConduitChatView.
///
/// ConduitChatView routes assistant content through `ConversationRenderer`
/// (block tokenizer) and `AttributedString(markdown:)` via the
/// `ConduitMarkdownBlock` view. The view itself isn't trivially testable
...
@Suite("ConduitChatView — markdown pipeline")
struct ConduitChatViewMarkdownTests {
    ...
    @Test @MainActor func messageRenderCacheKeysOnItemRevisionPair() {
        ...
        // ConduitMarkdownBlock's lookup contract holds.
